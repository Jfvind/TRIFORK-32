...
fn check_rust(sh: &Shell) -> Result<(), xshell::Error> {
    ...
    println!("--- Checking TRIFORK-32 HAL crate ---");
    cmd!(
        sh,
        "cargo check --package trifork32-hal --target {RUST_TARGET}"
    )
    .run()?;

    println!("--- Checking TRIFORK-32 runtime crate ---");
    cmd!(
        sh,
        "cargo check --package trifork32-rt --target {RUST_TARGET}"
    )
    .run()?;
...
fn docs(sh: &Shell) -> Result<(), xshell::Error> {
    println!("--- Building TRIFORK-32 HAL documentation ---");
    cmd!(
        sh,
        "cargo doc --package trifork32-hal --target {RUST_TARGET} --no-deps --open"
    )
    .run()?;
...
fn print_help() {
    ...
    println!("  docs              Build and open TRIFORK-32 HAL API documentation");
}
